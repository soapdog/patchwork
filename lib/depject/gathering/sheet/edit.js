...
const { Value, h, computed, when, send } = require("mutant");
...
exports.create = function (api) {
  ...
  return nest("gathering.sheet.edit", function (id) {
    getCurrentValues(id, (err, msg) => {
      ...
      const participants = msg.value && msg.value.content &&
      ...
      displaySheet((close) => {
          ...
          content: h("div", {
            ...
            h("GatheringEditor", [
              ...
              h("textarea.description", {
                ...
                hooks: [
                  ValueHook(chosen.description),
                  api.suggest.hook({ participants }),
                ],
              }),
            ]),
          ]),
        ...
        function save() {
          ...
          if (!compareImage(chosen.image(), msg.gathering.image)) {
            update.image = chosen.image();
          }
          if (
            !compareTime(chosen.startDateTime(), msg.gathering.startDateTime)
          ) update.startDateTime = chosen.startDateTime();
          if (chosen.title() !== msg.gathering.title) {
            update.title = chosen.title() || i18n("Untitled Gathering");
          }
          if (chosen.description() !== msg.gathering.description) {
            update.description = chosen.description();
          }
            ...
            api.sbot.async.publish(content, (err) => {
              if (err) {
                ...
                showDialog({
                  ...
                  buttons: ["OK"],
                  message: i18n(
                    "An error occurred while attempting to publish gathering.",
                  ),
                  detail: err.message,
                });
              } else {
    ...
    if (id) {
      api.sbot.async.get({ id, private: true }, (err, value) => {
        ...
        if (value.content.type === "gathering") {
          api.about.async.latestValues(id, [
            "title",
            "startDateTime",
            "image",
            "description",
