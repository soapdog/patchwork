...
exports.create = function (api) {
  ...
  function valueFrom(id, key, authorId) {
    ...
    return MutantPullValue(() => {
      return api.sbot.pull.stream((sbot) =>
        sbot.about.latestValueStream({ dest: id, key, authorId })
      );
    });
  }
  ...
  function latestValue(id, key) {
    ...
    return MutantPullValue(() => {
      return api.sbot.pull.stream((sbot) =>
        sbot.about.latestValueStream({ dest: id, key })
      );
    });
  }
  ...
  function socialValue(id, key, defaultValue) {
    ...
    if (!socialValueCache[id + "/" + key]) {
      const obs = socialValueCache[id + "/" + key] = MutantPullValue(() => {
        return api.sbot.pull.stream((sbot) =>
          sbot.about.socialValueStream({ dest: id, key })
        );
      }, {
        onListen: () => {
          socialValueCache[id + "/" + key] = obs;
        },
  ...
  function socialValues(id, key) {
    ...
    return MutantPullDict(() => {
      return api.sbot.pull.stream((sbot) =>
        sbot.about.socialValuesStream({ dest: id, key })
      );
    }, { checkDelete });
  }
