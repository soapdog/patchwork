...
const customScripts = {
	...
	copySamples: () => {
		...
		let sampleFiles = fs.readdirSync(sourceFolder, {
			withFileTypes: true,
		});
		...
		sampleFiles.forEach((f) => {
			...
			if (f.isFile()) {
				...
				fs.mkdirSync(path.join(samplesFolder, filename), {
					recursive: true,
				});
				sampleFiles = sampleFiles.concat(fs.readdirSync(path.join(samplesFolder, filename), {
					withFileTypes: true,
				}));
			}
		});
