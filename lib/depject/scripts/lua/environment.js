...
== Auxiliary functions ===========================================================================================================
  ...
  function markdown(text) {
    const htmlContent = ssbMarkdown.block(text);

    return htmlContent;
  }

  function openFileInWebView(file, opts) {
  ...
  function openWebView(html, opts) {
    if (electron?.ipcRenderer) {
      electron.ipcRenderer.send("open-custom-script-window", {
        html,
        opts,
    ...
    const wrapped = (L) => {
      ...
      if (res instanceof Promise && lua.lua_isyieldable(L)) { // if return a Promise and currently in a coroutine
        ...
        Promise.resolve(res).then((r) => {
          if (r === undefined) {
            lua.lua_resume(L, from, 0); // no return value
          } else {
...
== Implementation ===========================================================================================================
  */
  ...
  function init(path) {
    ...
    createSyncLuaFunction(L, "encodeJson", 1, encodeJson);
    createSyncLuaFunction(L, "decodeJson", 1, decodeJson);

    // - UI related
    createSyncLuaFunction(L, "openFileInWebView", 2, openFileInWebView);
    createSyncLuaFunction(L, "openWebView", 1, openWebView);
    createSyncLuaFunction(L, "markdown", 1, markdown);

    // - SSB related
    createAsyncLuaFunction(L, "getMessage", 1, getMessage);

    // - Async HTTP Requests
    createAsyncLuaFunction(L, "fetch", 1, luafetch);
    // END OF LUA API EXPOSED TO SCRIPTS
    ...
    // console.log(`[LUA] loaded script: ${path}`);
    return L;
  }
  ...
  function call(L, method, args = [], numberOfResults = 0) {
    // Get query to run
    // console.log(`[LUA] ${method} ${numberOfResults}`);
    lua.lua_getglobal(L, "script");
    if (!lua.lua_istable(L, -1)) {
      console.log(
        "query: lua script global is not a table:",
    ...
    if (lua.lua_pcall(L, args.length, numberOfResults, 0) !== lua.LUA_OK) {
      lauxlib.luaL_traceback(L, L, null, 1);
      console.log(`[LUA] Dumping stack after calling ${method}`);
      dumpstack(L);
      console.log(
        `[LUA] ${method} (results: ${numberOfResults}): error in lua pcall: ${
          lua.lua_tojsstring(L, -1)
        }`,
      );
