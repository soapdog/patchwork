...
exports.create = (api) => {
  ...
  return nest("message.html.actions", function like(msg, opts) {
    ...
    if (isFeatureEnabled("custom-scripts")) {
      ...
      for (const script of activeScripts) {
        ...
        const has = api.scripts.lua.environment.has(L, "button");
        if (!has) {
          continue;
        }
    ...
    return [
      ...
      isYourPost(msg)
        ? h("a.edit -right", {
          "ev-click": () => {
            api.sheet.editPost({ msg }, onEdited);
          },
        }, i18n("Edit"))
        : null,
      when(
  ...
  function isYourPost(msg) {
    if (
      msg.value?.content?.type == "post" &&
      msg.value.author == api.keys.sync.id()
    ) {
      return true;
    }
    return false;
