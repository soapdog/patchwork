const { h, Array: MutantArray, computed, Value, when, map } = require("mutant");
...
exports.create = function (api) {
  ...
  function layout(
    ...
    if (msg.value.content.root) {
      ...
      if (forkedFrom) {
        replyInfo = h("span", [
          i18n("forked from parent thread "),
          api.message.html.link(forkedFrom),
        ]);
      } else if (outOfContext) {
        replyInfo = h("span", [
          i18n("in reply to "),
          api.message.html.link(msg.value.content.root),
        ]);
      }
    } else if (msg.value.content.project) {
      replyInfo = h("span", [
        i18n("on "),
        api.message.html.link(msg.value.content.project),
      ]);
    }
    ...
    function editControl() {
      ...
      let bubbles = computed([edits, showing], (es, s) => {
        // check boundaries
        if (!es) return;
        if (s > es.length - 1) {
          showing.set(es.length - 1);
        ...
        // find content
        if (s === -1) {
          contentToShow.set(content); // back to original
        } else {
          const t = es[s].value.content.text;
          const md = api.message.html.markdown(t);
    ...
    function messageHeader(msg, { replyInfo, priority }) {
      ...
      if (priority === 2) {
        additionalMeta.push(
          h("span.flag -new", { title: i18n("New Message") }),
        );
      } else if (priority === 1) {
        additionalMeta.push(
          h("span.flag -unread", { title: i18n("Unread Message") }),
        );
      }
