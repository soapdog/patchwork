...
exports.create = function (api) {
  ...
  return nest("feed.html.rollup", function (getStream, {
    ...
    const updateLoader = h("a Notifier -loader", {
      href: "#",
      "ev-click": refresh,
    }, [
      "Show ",
      h("strong", [throttledUpdates]),
      " ",
      plural(throttledUpdates, i18n("update"), i18n("updates")),
    ]);
    ...
    if (updateStream) {
      pull(
        ...
        pull.drain((msg) => {
          if (
            !(msg && msg.value && msg.value.content && msg.value.content.type)
          ) return;
          ...
          if (
            msg.value.author !== yourId &&
            (!msg.root || canRenderMessage(msg.root))
          ) {
          ...
          if (msg.value.author === yourId && content()) {
            ...
            if (getRoot(msg)) {
              const existingContainer = content().querySelector(
                `[data-root-id="${getRoot(msg)}"]`,
              );
    ...
    const result = MutantArray([
      when(hidden, null, when(updates, updateLoader)),
      container,
    ]);
    ...
    function renderItem(item, opts) {
      ...
      let mostRecentBumpType = item.bumps && item.bumps[0] &&
        item.bumps[0].type;
      ...
      if (
        mostRecentBumpType !== "matches-channel" && item.rootBump &&
        item.rootBump.type === "matches-channel"
      ) {
        // the root post was in a channel that you subscribe to
        meta = h("div.meta", [
          many(item.rootBump.channels, channelLink, i18n),
          " ",
          i18n("mentioned in your network"),
        ]);
        ...
        if (mostRecentBumpType === "matches-channel") {
          ...
          bumps.forEach((bump) =>
            bump.channels && bump.channels.forEach((c) => channels.add(c))
          );
          meta = h("div.meta", [
            i18nPlural(
          ...
          const description = i18n(
            bumpMessages[mostRecentBumpType] || "added changes",
          );
          meta = h("div.meta", [
            many(authors, api.profile.html.person, i18n),
            " ",
            description,
          ]);
        }
      ...
      const hasMoreNewReplies = unreadBumps.some((id) =>
        !latestReplyIds.includes(id)
      );
        ...
        item.totalReplies > item.latestReplies.length
          ? h("a.full", {
            ...
            anchor: {
              ...
              anchor: hasMoreNewReplies
                ? unreadBumps[unreadBumps.length - 1]
                : null,
            },
        ...
        h("div.replies", [
          item.latestReplies.map((msg) => {
            ...
            return [
              ...
              api.message.html.missing(
                last(msg.value.content.branch),
                msg,
                item,
              ),
              result,
