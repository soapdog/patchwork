              ...
              pull(
                ...
                pullParallelMap(async (key, cb) => {
                  try {
                    ...
                    console.log("error getting message with key", key);
                    console.error(e);
                    cb(e, null);
                  }
                }),
                ...
                pull.collect((err, collectedMessages) => {
                  if (err) {
                    console.log("there's err", err);
                    console.log("cm", collectedMessages);
                    // reject(err);
                    resolve(
    ...
    */
    ...
    onceTrue(api.sbot.obs.connection, (s) => {
      ...
      popular({ period, page: 1 }).then((r) => {
        ...
        console.log("error fetching popular messages...");
        console.error(n);
        messages.set([]);
      });
    });
    ...
    const loading = computed([messages], (ms) => {
      return ms.length == 0;
    });
