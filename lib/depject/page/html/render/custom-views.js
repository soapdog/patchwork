    ...
    onceTrue(api.sbot.obs.connection, (sbot) => {
      pull(
        ...
        pull.drain((item) => {
          ...
          const returnType = lua.lua_tojsstring(L, -2)
          lua.lua_pop(L, 2)
          console.log("item from drain: " + returnType, q);
          items.push(q)
          x.set(x()+1)
      ...
      h("div.wrapper", [
        ...
        h(
          "section.content", map(items, i => {
            return api.message.html.render(i)
          }),
        ),
        when(
          loading,
