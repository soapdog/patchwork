...
function openAnnouncementsWindow() {
  let win = openWindow(
    null,
    Path.join(__dirname, "announcements-window.js"),
    {
      minWidth: 400,
      center: true,
      width: 400,
  ...
  win.setIcon(appIcon);
  // windows.win.openDevTools()
  return win;
}

function openAudioPlayer(ssbConfig, msg) {
  let display = electron.screen.getPrimaryDisplay();
  let width = display.bounds.width;
  let height = display.bounds.height;
  let win = openWindow(
    ssbConfig,
    Path.join(__dirname, "audio-player-window.js"),
    {
      minWidth: 100,
      center: true,
      width: 250,
...
function openCustomScriptWindow(ssbConfig, data) {
  let display = electron.screen.getPrimaryDisplay();
  let width = display.bounds.width;
  let height = display.bounds.height;
  let customScriptWindow = openWindow(
    ssbConfig,
    Path.join(__dirname, "custom-script-window.js"),
    {
      minWidth: 100,
      center: true,
  ...
  customScriptWindow.setIcon(appIcon);
  // win.customScriptWindow.openDevTools()
  return customScriptWindow;
}
...
function openProtocolGuideWindow() {
  ...
  let width = display.bounds.width;
  let height = display.bounds.height;
  let win = new electron.BrowserWindow(extend({
    show: false,
    resizable: true,
    devTools: true,
    minWidth: 1000,
...
function openIdentitiesManager() {
  ...
  win.on("focus", (ev) => {
    console.log("reload!");
    win.webContents.send("refresh-running-identities");
  });
...
function openEphemeralNetworksManager() {
  let display = electron.screen.getPrimaryDisplay();
  const win = openWindow(
    null, // no ssbConfig for identities manager.
    Path.join(__dirname, "ephemeral-networks-window.js"),
    {
      minWidth: 400,
      center: true,
      width: 1000,
      ...
      // titleBarStyle: "hiddenInset",
      autoHideMenuBar: true,
      title: "Ephemeral Networks Manager",
      webPreferences: {
        nodeIntegration: true,
        contextIsolation: false,
      },
  ...
  win.on("focus", (ev) => {
    win.webContents.send("refresh-ephemeral-networks");
  });
...
async function startServerForIdentity(identity) {
  ...
  if (configuration?.ephemeral) {
    ssbConfig.caps.shs = configuration?.ephemeral?.caps?.shs;
    ssbConfig.ephemeral = configuration?.ephemeral;
  }

  if (configuration?.remote) {
    ssbConfig.remote = `${configuration.remote}~shs:${pubkey}`;
