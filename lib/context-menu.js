...
const luaEnvironment = require("./depject/scripts/lua/environment.js");
...
module.exports = function (
  ...
  ipcMain.removeHandler("context-menu-info"); // needed cause this may be called more than once.
    ...
    menu: (defaultActions, parameters, _, dictionarySuggestions) => {
      ...
      if (isFeatureEnabled("custom-scripts")) {
        ...
        for (const script of activeScripts) {
          ...
          const [r, label] = api.scripts.lua.environment.call(
            L,
            "contextMenu",
            [msgKey],
            2,
          ...
          if (label) {
            ...
            customMenus.push({
              label: label,
              visible: !!hasCustomScripts && !!msgKey,
              click: function () {
                window.webContents.send("execute-lua", {
                  script: script.name,
...
function copySSBURI(window) {
  const msgKey = lastClickInfo?.msg?.key;
  const url = ssbUri.fromMessageSigil(msgKey);
  return {
    label: "Copy SSB URL",
    visible: !!msgKey,
    click: function () {
...
function findRefs(parameters, navigate) {
  const extractedRef = parameters.mediaType === "none"
  ...
  const label = !!extractedRef
    ? `Find ${usageOrRef} ${
      extractedRef.slice(0, 10).replaceAll("&", "&&&")
    }...`
    : "";
  return {
    label,
