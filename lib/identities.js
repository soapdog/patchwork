...
function createDefaultIdentity() {
  ...
  if (!fs.existsSync(defaultIdentityPath)) {
    fs.ensureDirSync(path.basename(defaultIdentityPath));
...
function createEphemeral(network) {
  const tempPath = path.join(identitiesFolder, "temp");
  const secretPath = path.join(tempPath, "secret");
  ...
  const configFile = path.join(newPath, "ponchowonky.toml");
  const config = Object.assign({}, minimalConfig);

  config.ephemeral = network;

  fs.writeFileSync(configFile, toml.stringify(config));

  return { public: keys.public, keys, path: newPath };
}
...
function listEphemeral() {
  ...
  const isEphemeralNetworkIdentity = (i) => {
    const configFile = path.join(i, "ponchowonky.toml");

    if (fs.existsSync(configFile)) {
      const config = toml.parse(fs.readFileSync(configFile));

      if (Object.hasOwn(config, "ephemeral")) {
        return true;
      }
    }
  ...
  if (fs.existsSync(identitiesFolder)) {
    ...
    identities = everything
      .map((i) => path.join(identitiesFolder, i))
      .filter(isDirectory)
      .filter(isEphemeralNetworkIdentity)
      .map((f) => {
        const s = path.join(f, "secret");
        const keys = ssbKeys.loadOrCreateSync(s);
        return { keys, path: f };
...
function getIdentityForEphemeralNetwork(network) {
  const identities = listEphemeral();
  var retVal = null;

  for (const i of identities) {
    console.log(JSON.stringify(network, null, 2));

    const config = configurationForIdentity(i.keys.id);
    console.log(JSON.stringify(config, null, 2));

    if (config.ephemeral.caps.shs === network.caps.shs) {
      retVal = i;
    }
  }
...
module.exports = {
  ...
  createEphemeral,
  listEphemeral,
  getIdentityForEphemeralNetwork,
};
