const { h, Value, Dict, dictToCollection, map, computed, resolve, watch } =
  require("mutant");
...
module.exports = function (renderPage, defaultViews) {
    ...
    views.keys().forEach((view) => {
      if (!(resolve(defaultViews) || []).includes(view)) {
        if (
          ...
          Date.now() - lastViewed[view] > (5 * 60e3) && view !== currentView()
        ) {
          views.delete(view);
        }
      }
  ...
  function deleteView(view, anchor) {
  ...
  function setView(view, anchor) {
    ...
    if (views.has(view)) {
      ...
      // delete old searches, save memory
      views.keys().forEach((key) => {
        if (key.startsWith("?") && key !== view) {
          console.log(`removing old search: ${key}`);
          views.delete(key);
        }
      });
