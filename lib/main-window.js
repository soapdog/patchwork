...
module.exports = function (config) {
  ...
  const includeParticipating = api.settings.obs.get(
    "patchwork.includeParticipating",
    false,
  );
  const includePopular = api.settings.obs.get(
    "patchwork.includePopular",
    false,
  );
  const autoDeleteBlocked = api.settings.obs.get(
    "patchwork.autoDeleteBlocked",
    false,
  );
        ...
        .forEach((feed) => {
          pull(
            ...
            pull.collect((err, keys) => {
              ...
              if (keys.length > 0) {
                console.log(
                  `deleted ${keys.length} messages from blocked authors`,
                );
              }
            }),
          );
  ...
  const defaultViews = computed(
    [includeParticipating, includePopular],
    (includeParticipating, includePopular) => {
      const result = [
        "/public",
        "/private",
      ...
      if (includePopular) {
        result.push("/popular?period=week");
      }
  ...
  electron.ipcRenderer.on("execute-lua", (_ev, data) => {
    const L = api.scripts.lua.environment.init(data.script);
    const has = api.scripts.lua.environment.has(L, data.method);

    if (!has) {
      return;
    }

    api.scripts.lua.environment.call(
      L,
      data.method,
      data.args,
      data.returnValues,
    ...
    h("style", {
      innerHTML: computed(
        ...
        (themeName) => {
          ...
          const syntaxTheme = syntaxThemeOptions[themeName] ||
            syntaxThemeOptions.light;
  ...
  const container = h(`MainWindow -${process.platform}`, {
    ...
    h("div.top", [
      ...
      h("span.appTitle", [
        h(
          "span.title",
          isFeatureEnabled("multiple-identities")
            ? [
              `Poncho Wonky of `,
              api.about.obs.name(id),
              config?.ephemeral ? ` at ${config?.ephemeral?.name}` : null,
            ]
            : "Poncho Wonky",
        ),
      ...
      h("span.nav", [
        ...
        computed(includeParticipating, (includeParticipating) => {
          if (includeParticipating) {
            return tab(i18n("Participating"), "/participating");
          }
        }),
        computed(includePopular, (includePopular) => {
          if (includePopular) {
            const menu = [
              {
                type: "normal",
                label: i18n("Week"),
                target: "/popular?period=week",
              },
              {
                type: "normal",
                label: i18n("Month"),
                target: "/popular?period=month",
              },
              {
                type: "normal",
                label: i18n("Year"),
                target: "/popular?period=year",
              },
            ];
            const menuElement = h("a -drop", {
              "ev-click": () => {
                const rects = menuElement.getBoundingClientRect();
    ...
    when(
      ...
      h("div.info", [
        h("a.message -update", {
          href: "https://github.com/soapdog/patchwork/releases/latest",
        }, [
          h("strong", [
            "Poncho Wonky ",
            latestUpdate,
            i18n(" has been released."),
          ]),
          i18n(" Click here to download and view more info!"),
  ...
  function buildDropdownMenuItems() {
    // vars need to be out of the clause due to scope
    let customMenus = [];
    let hasCustomScripts = false;
    if (isFeatureEnabled("custom-scripts")) {
      const activeScripts = customScripts.getActiveScriptsList();

      let customScriptsMenu;
      ...
      for (const script of activeScripts) {
        ...
        const [label] = api.scripts.lua.environment.call(
          L,
          "menu",
          [],
          1,
        ...
        if (label) {
          ...
          customMenus.push({
            type: "normal",
            label: label,
            target: "/custom-script?action=menuAction&script=" + script.name,
          });
        }
      }
    }
    const dropTabItems = [
      ...
      {
        type: "normal",
        label: i18n("Post by order of arrival"),
        target: "/order-of-arrival",
      },
      {
        type: "normal",
        label: i18n("Books"),
      ...
      isFeatureEnabled("multiple-identities")
        ? {
          type: "event",
          label: i18n("Ephemeral Networks"),
          target: "!ephemeral-networks",
        }
        : null,
    ];
  ...
  function tab(name, view) {
    ...
    return h("a", {
      "ev-click": function () {
        ...
        const needsRefresh = instance && instance.pendingUpdates &&
